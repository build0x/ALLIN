...
const SelectTableModal = ({ mode, context, closeModal }) => {
  ...
  const filterList = [
    { key: 'allRB', label: t('ALL'), params: 'all' },
    { key: 'lowRB', label: t('LOW_BETS'), params: 'lowBets' },
    { key: 'mediumRB', label: t('MEDIUM_BETS'), params: 'mediumBets' },
    { key: 'highRB', label: t('HIGH_BETS'), params: 'highBets' },
  ];
  ...
  const getRooms = (socket, tableSortParam) => {
    if (socket) {
      if (tableId === -1) {
        ...
        toast.warn(t('ROOM_ALREADY_JOINED_RELOAD'));
  ...
  const RoomView = useMemo(() => {
    ...
    return roomsData.map((rData) => {
      ...
      const desc = ` ➟ ${rData.playerCount}/${rData.maxSeats} ➟ ${t('MIN_BET_SHORT')} ${minBet}`;
      return (
        <button
          key={rData.tableId}
          type="button"
